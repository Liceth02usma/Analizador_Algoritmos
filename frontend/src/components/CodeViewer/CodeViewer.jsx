...
export default function CodeViewer({ code, explanation, complexity }) {
  ...
  // Verificar si el código contiene múltiples casos (formato con === MEJOR CASO ===)
  const hasMultipleCases = typeof code === 'string' && code.includes('=== MEJOR CASO ===');

  if (hasMultipleCases) {
    // Parsear el código por casos
    const bestCaseMatch = code.match(/=== MEJOR CASO ===([\s\S]*?)(?:=== PEOR CASO ===|$)/);
    const worstCaseMatch = code.match(/=== PEOR CASO ===([\s\S]*?)(?:=== CASO PROMEDIO ===|$)/);
    const averageCaseMatch = code.match(/=== CASO PROMEDIO ===([\s\S]*?)$/);
    ...
    const cases = [
      { 
        ...
        label: "🟢 Mejor Caso", 
        color: "bg-green-600",
        code: bestCaseMatch ? bestCaseMatch[1].trim() : ""
      },
      { 
        type: "worst_case", 
        label: "🔴 Peor Caso", 
        color: "bg-red-600",
        code: worstCaseMatch ? worstCaseMatch[1].trim() : ""
      },
      { 
        type: "average_case", 
        label: "🟡 Caso Promedio", 
        color: "bg-yellow-600",
        code: averageCaseMatch ? averageCaseMatch[1].trim() : ""
      }
    ].filter(c => c.code);
    ...
    return (
      <div className="bg-gray-800 rounded-xl p-6 space-y-4">
        ...
        {/* Explicación general */}
        {explanation && activeCase === 0 && (
          <div className="bg-gray-700 p-4 rounded-lg">
            <h4 className="text-lg font-semibold text-gray-200 mb-2">Descripción del Algoritmo</h4>
            <p className="text-gray-300">{explanation}</p>
          </div>
        )}

        {/* Código anotado */}
        <CodeBlock code={currentCase.code} />

        {/* Complejidad por caso */}
        {complexity && (
          <div className="bg-gray-700 p-4 rounded-lg">
            <h4 className="text-lg font-semibold text-gray-200 mb-2">Explicación de Complejidad</h4>
            <div className="text-gray-300 whitespace-pre-line">{complexity}</div>
          </div>
        )}
      </div>
    );
  ...
  // Si code es un array de objetos (múltiples casos) - Formato antiguo
