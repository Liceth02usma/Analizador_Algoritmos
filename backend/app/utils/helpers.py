...
def generate_recurrence_tree_mermaid(
    response: "RecurrenceTreeResponse",
) -> Dict[str, str]:
    """
    ...
    for tree_idx, tree in enumerate(response.trees):
        ...
        for level in sorted_levels:
            ...
            if level > 0:
                ...
                child_cursor = (
                    0  # Puntero para recorrer los hijos disponibles en el nivel actual
                )
...
def generate_tree_method_diagram(result: Dict[str, any]) -> str:
    ...
    if len(levels) > max_levels:
        mermaid_lines.append(
            f'    more["... {len(levels) - max_levels} niveles más ..."]'
        )
...
def generate_tree_visualization(
    data: Union["RecurrenceTreeResponse", Dict[str, any]],
