...
class AlgorithmKnowledgeBase:
            ...
            metadatas.append(
                {
                    ...
                    "keywords": algo[
                        "keywords"
                    ],  # Guardamos keywords para búsqueda exacta
                }
            )
    ...
    def search_algorithm(
        ...
        for algo in KNOWN_ALGORITHMS:
            ...
            keyword_match = any(
                k.strip() in query_lower for k in algo["keywords"].split(",")
            )
        ...
        if distance < threshold:
            ...
            print(
                f"🎯 [KnowledgeBase] Match semántico encontrado: {metadata['official_name']} (Dist: {distance:.3f})"
            )
            return metadata["pseudocode"]

        print(
            f"⚠️ [KnowledgeBase] Sin coincidencias suficientes (Mejor: {distance:.3f} > {threshold})"
        )
        return None
