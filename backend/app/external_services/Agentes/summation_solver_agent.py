...
from app.external_services.Agentes.explanation_agent import (
    MathematicalExplanationAgent,
    ExplanationResponse,
)
...
class SummationSolverInput(BaseModel):
    algorithm_name: str
    cases: List[SolverCaseInput] = Field(
        description="Lista de casos con sus sumatorias"
    )
...
class HybridSummationSolverAgent(AgentBase[HybridSolverResponse]):
    ...
    def __init__(self, model_type: str, *args, **kwargs):
        ...
        self.explainer = MathematicalExplanationAgent(model_type=self.model_type)
...
"""
    ...
    def solve_summations(
        self, algorithm_name: str, cases_data: List[Dict[str, Any]]
        ...
        for idx, case in enumerate(cases_data, 1):
            ...
            c_sum = (
                case.get("solver_friendly_summation")
                if isinstance(case, dict)
                else case.solver_friendly_summation
            )
        ...
        try:
            explanations = self.explainer.explain_solution(
                algorithm_name=algorithm_name, cases_data=sympy_results
            )
            ...
            explanations = self._create_fallback_explanations(
                sympy_results, algorithm_name
            )
        ...
        for sympy_result, explanation in zip(
            sympy_results, explanations.cases_explanations
        ):
            ...
            solved_cases.append(
                SolvedCaseResult(
                    ...
                    big_o_notation=sympy_result[
                        "big_o"
                    ],  # Usamos Big-O de SymPy (más confiable)
    ...
    def _create_fallback_explanations(
        self, sympy_results: List[Dict], algorithm_name: str
    ):
        """
        ...
        for result in sympy_results:
            steps = [
                ExplanationStep(
                    ...
                    mathematical_expression=step.get(
                        "expression", step.get("result", "")
                    ),
        ...
        return ExplanationResponse(
            algorithm_name=algorithm_name, cases_explanations=cases_explanations
        )

    def _generate_general_summary(
        self, cases: List[SolvedCaseResult], algorithm_name: str
    ) -> str:
        """
        ...
        summary_parts = [
            f"El algoritmo {algorithm_name} presenta diferentes complejidades según el caso:"
        ]
...
if __name__ == "__main__":
    try:
        ...
        mock_insertion_sort_cases = [
            {"case_name": "Mejor Caso", "solver_friendly_summation": "c1 + c2*n + c3"},
            {
                "case_name": "Peor Caso",
        ...
        solution = hybrid_solver.solve_summations(
            algorithm_name="Insertion Sort", cases_data=mock_insertion_sort_cases
        )
