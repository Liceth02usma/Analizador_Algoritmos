...
class ExplanationStep(BaseModel):
    ...
    step_number: int
    title: str = Field(description="Título del paso. Ej: 'Identificar sumatorias anidadas'")
    explanation: str = Field(description="Explicación en lenguaje natural")
    mathematical_expression: str = Field(description="Expresión matemática de este paso")
    property_or_formula: Optional[str] = Field(default=None, description="Propiedad matemática usada")
...
class CaseExplanation(BaseModel):
    ...
    summary: str = Field(
        description="Resumen final explicando el resultado"
    )
...
""")
            for step in case['steps']:
                ...
                if 'formula_applied' in step:
                    formatted.append(f"  └─ Fórmula aplicada: {step['formula_applied']}")
                if 'result' in step:
