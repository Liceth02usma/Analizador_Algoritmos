...
class Recursive(Algorithm):
    ...
    def __init__(self, name: str, pseudocode: str):
        ...
        self.type = "Recursivo"
        self.base_case_condition: str = ""  # Condición del caso base
        self.recurrence_relation: str = ""  # Relación de recurrencia T(n) = ...
        self.recursion_depth: int = 0  # Profundidad máxima de recursión
        self.recursive_calls: int = 0  # Número de llamadas recursivas por invocación
        self.recursive_call_nodes: List[Dict[str, Any]] = []  # Nodos de llamadas recursivas
    ...
    def extract_recurrence(self) -> str:
        ...
        """
        # Encontrar llamadas recursivas en el AST
        self.recursive_call_nodes = []
        
        def find_recursive_calls(node: Any):
            """Encuentra todas las llamadas recursivas al mismo procedimiento."""
            if isinstance(node, dict):
                # Detectar llamadas al mismo procedimiento (recursión)
                if node.get('type') == 'call' and node.get('name', '').lower() == self.name.lower():
        ...
        if self.recursive_calls == 0:
            # No hay recursión detectada
            self.recurrence_relation = "T(n) = O(1)"
            return self.recurrence_relation
        
        # Analizar los argumentos de las llamadas recursivas para determinar el patrón
        ...
        for call_node in self.recursive_call_nodes:
            ...
            for arg in args:
                ...
                # Detectar división (n/2, n/b)
                if '/' in arg_str or 'div' in arg_str:
                    division_pattern = True
                    # Intentar extraer el factor
                    if '/2' in arg_str or '/ 2' in arg_str:
                        divide_by_factor = 2
                    elif '/3' in arg_str or '/ 3' in arg_str:
                        divide_by_factor = 3
                # Detectar resta (n-1, n-k)
                elif '-' in arg_str:
        ...
        if division_pattern:
            ...
            if self.recursive_calls == 1:
                ...
                # Múltiples llamadas recursivas con resta
    ...
    def build_recursion_tree(self) -> Tree:
        ...
        max_depth = min(self.recursion_depth, 4)  # Limitar para no crear árboles muy grandes
        
        self._build_tree_recursive(tree, "T(n)", 0, max_depth)
        
        return tree

    def _build_tree_recursive(self, tree: Tree, node: str, current_depth: int, max_depth: int) -> None:
        """
    ...
    def identify_pattern(self) -> AlgorithmPatterns:
        ...
        # Identificar patrones comunes basados en la relación de recurrencia
        if '/2' in self.recurrence_relation or 'divide' in ' '.join(self.tokens).lower():
            pattern.pattern_name = "Divide y Conquista"
            pattern.description = "Algoritmo que divide el problema en subproblemas más pequeños"
            pattern.complexity_hint = "O(n log n) típicamente"
            pattern.examples = ["Merge Sort", "Quick Sort", "Búsqueda Binaria"]
        
        elif 'fibonacci' in self.name.lower() or (self.recursive_calls >= 2 and '-' in self.recurrence_relation):
            pattern.pattern_name = "Recursión Múltiple"
            pattern.description = "Múltiples llamadas recursivas, típico de Fibonacci"
            pattern.complexity_hint = "O(2^n) sin memorización"
            pattern.examples = ["Fibonacci", "Torres de Hanoi"]
        
        elif 'T(n-1)' in self.recurrence_relation or 'T(n-2)' in self.recurrence_relation:
            pattern.pattern_name = "Recursión Lineal"
            pattern.description = "Reduce el problema en una unidad en cada llamada"
            pattern.complexity_hint = "O(n)"
            pattern.examples = ["Factorial", "Suma de array recursiva"]
        
        elif self.recursive_calls == 0:
            pattern.pattern_name = "Sin Recursión"
            pattern.description = "No se detectaron llamadas recursivas"
            pattern.complexity_hint = "O(1) o iterativo"
    ...
    def extract_base_case(self) -> List[Dict[str, Any]]:
        ...
        def find_base_case(node: Any, path: str = "root") -> None:
            ...
            if isinstance(node, dict):
                # Buscar sentencias if que contengan return directo
                if node.get('type') == 'if':
                    then_body = node.get('then', [])
                    
                    # Verificar si el then contiene un return directo
                    has_return = False
                    if isinstance(then_body, list):
                        for stmt in then_body:
                            if isinstance(stmt, dict) and stmt.get('type') == 'return':
                                has_return = True
                                break
                    elif isinstance(then_body, dict) and then_body.get('type') == 'return':
                        has_return = True
