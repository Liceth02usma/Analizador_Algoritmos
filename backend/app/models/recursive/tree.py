...
class TreeNode(BaseModel):
    ...
    children_count: int = Field(
        description="Número de hijos directos de este nodo."
    )
...
class RecurrenceTreeResponse(BaseModel):
    ...
    summary: str = Field(
        description="Resumen general de los árboles generados."
    )
...
class RecurrenceInput(BaseModel):
    ...
    best_case: Optional[str] = Field(
        None,
        description="Ecuación del mejor caso si existe."
    )
    worst_case: Optional[str] = Field(
        None,
        description="Ecuación del peor caso si existe."
    )
    average_case: Optional[str] = Field(
        None,
        description="Ecuación del caso promedio si existe."
    )
    single_equation: Optional[str] = Field(
        None,
        description="Ecuación única si no hay casos múltiples."
    )
    max_depth: int = Field(
        default=4,
        description="Profundidad máxima del bosquejo del árbol."
    )
...
**IMPORTANTE:** Solo generar la estructura visual, NO calcular costos."""
            ...
            if self.enable_verbose:
                ...
                for tree in response.trees:
                    print(f"    • {tree.case_type}: {tree.description} ({len(tree.tree_structure)} nodos)")
