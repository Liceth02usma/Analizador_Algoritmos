...
class CaseDetectionAgent(AgentBase[CaseDetectionResponse]):
    ...
    def __init__(
        self, model_type: str = "Gemini_Rapido", provider: Optional[str] = None
    ):
...
**Sé breve. Solo marca true si hay evidencia clara de complejidad variable."""
    ...
    def detect_cases(
        ...
        if any(
            keyword in pseudocode_lower or keyword in name_lower
            ...
            print(f"\n📝 Razonamiento:")
            print(
                f"   QuickSort detectado: pivote causa particiones desbalanceadas → múltiples casos"
            )
            print(f"\n🔎 Patrones detectados:")
...
Retorna tu análisis en formato JSON."""
        ...
        # Invocar el agente
        result = self.invoke_simple(content=user_message, thread_id=thread_id)
        ...
        if response is None:
            print(
                "⚠️ [CaseDetectionAgent] No se pudo parsear la respuesta. Asumiendo caso general."
            )
            return False
        ...
        print(f"{'='*70}")
        print(
            f"✓ Tiene múltiples casos: {'SÍ' if response.has_multiple_cases else 'NO'}"
        )
        print(f"\n📝 Razonamiento:")
...
Retorna tu análisis en formato JSON."""
        ...
        if response is None:
            ...
            print(
                "⚠️ [CaseDetectionAgent] No se pudo parsear la respuesta. Retornando caso general."
            )
