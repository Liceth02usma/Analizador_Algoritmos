...
class Solution(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        if self.explain_solution_steps:
            ...
            if self.explain_solution_steps and isinstance(self.explain_solution_steps[0], dict):
                for i, analysis in enumerate(self.explain_solution_steps, 1):
                    ...
                    if analysis.get('classification_confidence'):
                        lines.append(f"      Classification Confidence: {analysis['classification_confidence']:.2f}")
                    
                    if analysis.get('explanation'):
                        lines.append(f"      Explanation: {analysis['explanation'][:150]}...")
                    
                    if analysis.get('steps'):
                        lines.append(f"      Resolution Steps: {len(analysis['steps'])} steps")
        ...
        if self.diagrams:
            ...
            # Recursion trees
            if "recursion_trees" in self.diagrams:
                trees = self.diagrams["recursion_trees"]
                if hasattr(trees, 'trees'):
                    lines.append(f"  Recursion Tree Sketches: {len(trees.trees)} generated")
                    for tree in trees.trees:
                        lines.append(f"    - {tree.case_type}: {tree.description}")
                lines.append("")
            
            # Tree method details
            for key, value in self.diagrams.items():
                if key.startswith("tree_method_"):
                    case_type = key.replace("tree_method_", "").replace("_", " ").title()
                    lines.append(f"  Tree Method Expansion ({case_type}):")
                    if isinstance(value, dict):
                        lines.append(f"    Equation: {value.get('equation', 'N/A')}")
                        lines.append(f"    Tree Depth: {value.get('tree_depth', 'N/A')}")
                        if value.get('levels_expansion'):
                            lines.append(f"    Levels: {len(value['levels_expansion'])} levels")
                    lines.append("")
