...
def test_basico_1_fibonacci_recursivo():
    ...
    assert (
        result["algorithm_type"] == "recursivo"
    ), f"❌ Esperado: recursivo, Obtenido: {result['algorithm_type']}"
...
def test_intermedio_1_binary_search_recursivo():
    ...
    ast = [
        {
            ...
            "body": [
                ...
                },
                {
                    ...
                    "value": {
                        "op": "//",
                        "lhs": {"op": "+", "lhs": "left", "rhs": "right"},
                        "rhs": 2,
                    },
                },
                {
                    "type": "if",
                    "cond": {"op": "==", "lhs": "arr[mid]", "rhs": "target"},
    ...
    assert result["algorithm_type"] == "recursivo"
    print(
        f"✅ INTERMEDIO-1: Binary Search Recursivo → {result['algorithm_type'].upper()}"
    )
    return True
...
def test_intermedio_2_merge_sort_recursivo():
    ...
    assert result["algorithm_type"] == "recursivo"
    print(
        f"✅ INTERMEDIO-2: Merge Sort → {result['algorithm_type'].upper()} (recursión domina sobre bucles)"
    )
    return True
            ...
            "body": [
                {
                    ...
                    "var": "j",
                    "start": "low",
                    "end": {"op": "-", "lhs": "high", "rhs": 1},
                    "body": [
                        {
                            "type": "assign",
                            "lvalue": "i",
                            "value": {"op": "+", "lhs": "i", "rhs": 1},
                        }
                    ],
                },
...
def test_intermedio_5_suma_arreglo_recursivo():
    ...
    assert result["algorithm_type"] == "recursivo"
    print(
        f"✅ INTERMEDIO-5: Suma Arreglo Recursivo → {result['algorithm_type'].upper()}"
    )
    return True
...
def test_avanzado_1_recursion_mutua():
    ...
    assert result["algorithm_type"] == "recursivo"
    assert (
        len(result["recursive_functions"]) >= 1
    )  # Al menos una función debe estar marcada
    print(
        f"✅ AVANZADO-1: Recursión Mutua (isEven/isOdd) → {result['algorithm_type'].upper()}"
    )
    return True
            ...
            "body": [
                {
                    ...
                    "cond": {"op": "==", "lhs": "m", "rhs": 0},
                    "then": [
                        {"type": "return", "value": {"op": "+", "lhs": "n", "rhs": 1}}
                    ],
                    "else": [
                        {
                            "type": "if",
...
def test_avanzado_3_recursion_triple_cadena():
    ...
    assert result["algorithm_type"] == "recursivo"
    print(
        f"✅ AVANZADO-3: Recursión Triple Cadena (A→B→C→A) → {result['algorithm_type'].upper()}"
    )
    return True
...
def test_avanzado_4_dfs_grafo_recursivo():
            ...
            "body": [
                {"type": "assign", "lvalue": "visitados[nodo]", "value": True},
                {
                    ...
                    "body": [
                        {
                            "type": "if",
                            "cond": {"op": "not", "value": "visitados[vecino]"},
                            "then": [
                                {
                                    "type": "call",
                                    "name": "dfs",
                                    "args": ["grafo", "vecino", "visitados"],
                                }
                            ],
                        }
                    ],
    ...
    assert result["algorithm_type"] == "recursivo"
    print(
        f"✅ AVANZADO-4: DFS Grafo → {result['algorithm_type'].upper()} (recursión + bucle)"
    )
    return True
...
def test_avanzado_5_permutaciones_recursivo():
    ...
    assert result["algorithm_type"] == "recursivo"
    print(
        f"✅ AVANZADO-5: Permutaciones (Backtracking) → {result['algorithm_type'].upper()}"
    )
    return True
...
def test_extremo_1_funcion_vacia():
    ...
    ast = [{"type": "procedure_def", "name": "empty", "params": [], "body": []}]

    classifier = AlgorithmClassifier()
    result = classifier.classify(ast)

    assert (
        result["algorithm_type"] == "iterativo"
    )  # Por defecto, sin recursión = iterativo
...
def test_extremo_3_bucles_multiples_iterativo():
                    ...
                    "body": [
                        {
                            "type": "assign",
                            "lvalue": "n",
                            "value": {"op": "-", "lhs": "n", "rhs": 1},
                        }
                    ],
                },
                {
                    "type": "repeat_loop",
                    "body": [{"type": "print", "value": "done"}],
                    "until": {"op": "true"},
                },
            ],
        }
    ...
    assert result["algorithm_type"] == "iterativo"
    print(
        f"✅ EXTREMO-3: Bucles Múltiples Secuenciales → {result['algorithm_type'].upper()}"
    )
    return True
...
def test_extremo_4_llamadas_externas_iterativo():
    ...
    assert result["algorithm_type"] == "iterativo"
    print(
        f"✅ EXTREMO-4: Llamadas Externas → {result['algorithm_type'].upper()} (no son auto-recursivas)"
    )
    return True
...
def test_extremo_5_recursion_condicional_compleja():
            ...
            "body": [
                ...
                },
                {
                    ...
                    "else": [
                        {
                            "type": "while_loop",
                            "cond": {"op": ">", "lhs": "n", "rhs": 10},
                            "body": [
                                {
                                    "type": "call",
                                    "name": "complexRecursive",
                                    "args": [{"op": "//", "lhs": "n", "rhs": 2}, True],
                                }
                            ],
                        }
                    ],
    ...
    assert result["algorithm_type"] == "recursivo"
    print(
        f"✅ EXTREMO-5: Recursión Condicional Compleja → {result['algorithm_type'].upper()}"
    )
    return True
...
def test_extremo_6_floyd_warshall_iterativo():
    ...
    assert result["algorithm_type"] == "iterativo"
    print(
        f"✅ EXTREMO-6: Floyd-Warshall → {result['algorithm_type'].upper()} (3 bucles, 0 recursión)"
    )
    return True
